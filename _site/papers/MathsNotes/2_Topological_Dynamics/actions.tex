...
\((X,\mathscr{B})\) is a measurable mapping
...
\item
  ...
  \(\GroupElement.(\GroupElement'.x)=\GroupAction{(\GroupOperation{\GroupElement}{\GroupElement'})}{x}\)
\item
  Identity: There exists an identity element \(\GroupIdentity\in\Group\)
  such that \(\GroupAction{\GroupIdentity}{x}=x\) for all \(x\in X\).
\item
  Quasi-Invariance: For any \(B\in\mathscr{B}\) and for all
  \(\GroupElement\in\Group\), we have
  \(\mu(\GroupAction{\GroupElement}{B})=0\) if and only if \(\mu(B)=0\).
\end{enumerate}
...
\tightlist
\item
  If \(B\in\mathscr{B}\) and
  \(\mu(B)=\mu(\GroupAction{\GroupElement}{B})\) for any
  \(\GroupElement\in\Group\), then \(\mu(B)=0\) or
  \(\mu(X\setminus B)=0\).
\end{enumerate}
